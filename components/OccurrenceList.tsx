...
const OccurrenceList: React.FC = () => {
    const { occurrences } = useDataContext();
    const { openOccurrenceModal } = useUIContext();
    const { deleteOccurrence, handleDataImport } = useActionsContext();
    const importInputRef = useRef<HTMLInputElement>(null);

     const handleExport = () => {
        const success = exportToCSV(occurrences, 'ocorrencias.csv');
        if (success) {
            toast.success('Ocorrências exportadas com sucesso!');
        } else {
            toast.error('Não há ocorrências para exportar.');
        }
    };
    ...
    const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                handleDataImport('occurrences', jsonData);
            } catch (error) {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-xl shadow-lg">
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Tipos de Ocorrência</h1>
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead className="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Nome da Ocorrência</th>
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {occurrences.map((occ) => (
                            <tr key={occ.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">{occ.name}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                     <button onClick={() => openOccurrenceModal(occ)} className="text-blue-500 hover:text-blue-700 p-1.5 rounded-full hover:bg-blue-100 dark:hover:bg-gray-600" title="Editar">
                                        <EditIcon className="h-5 w-5" />
                                    </button>
                                     <button onClick={() => deleteOccurrence(occ.id)} className="text-red-500 hover:text-red-700 p-1.5 rounded-full hover:bg-red-100 dark:hover:bg-gray-600" title="Excluir">
